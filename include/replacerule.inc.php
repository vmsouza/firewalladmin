...
function ReplaceRule($type,$table,$chain,$id,$lang) {
	$id++;
	echo "<form method=post action=\"$PHP_SELF\">"
	    ."<table border=0 cellspacing=2 cellpadding=2 width=100%>"
	    ."<tr bgcolor=darkorange><td colspan=2><b>".strtoupper($lang["chruleheader"])."<big></b></td></tr>"
	...
	if ($type=="replace") {
		echo "<tr><td>".$lang["position"].":</td><td>$id</td></tr>";
	}
...
//	exit;
	...
	echo "</select></td></tr>"
    	    ...
    	    ."<input type=hidden name=op value=replace2>"
	    ."</table>"
	    ."</form>";
}

function ReplaceRule2($type,$table,$chain,$max,$ifacein,$ifaceout,$position,$protocol,$modules,$lang) {
	if ($type=="insert")
	...
	echo "<input type=hidden name=op value=replace3>";
	echo "</table>";
}

function ReplaceRule3($type,$table,$chain,$ifacein,$ifaceout,$position,$protocol,$modules,$selmodules,$targets,$lang,$ipt) {
	if ($type=="replace")
		$positiondesc="(".$lang["position"]." $position)";
	else
		$positiondesc="";
	...
	echo "<input type=hidden name=chain value=$chain>";
	echo "<input type=hidden name=position value=$position>";
	echo "<input type=hidden name=op value=replace4>";
	echo "<tr><td colspan=2><input type=submit name=submit value=\"".$lang["replace"]."\"></td></tr>";
	echo "</form>";
	echo "</table>";
}

function ReplaceRule4($type,$table,$chain,$ifacein,$ifaceout,$position,$protocol,$moduleoption,$lang,$iptables,$target,$rejectwith,$logprefix,$toports,$dnat,$snat,$tos,$mark,$dscp,$dscpclass) {
	$cmd="$iptables -t $table ";
	$cmd.="-R $chain $position ";
	if ($protocol!="all")
		$cmd.="-p $protocol ";
	...
	echo "<table border=0 cellspacing=2 cellpadding=2 width=100%>"
	    ."<tr bgcolor=darkorange><td colspan=2><b>".$lang["chruleheader"]."</b></td></tr>";
	if ($return==0) {
